...
- **Why it defaults to OFF:** the module is loaded on *every* request, whether or not you run tournaments live. Measured on a real WordPress 7.1 install (admin request, 128 MB limit): WordPress core alone peaks at 38912 KB; adding this plugin with the module ON costs 8704 KB, with it OFF only 4608 KB. **The gate halves the plugin's memory cost and saves 4096 KB per request.** Reachable files drop from 93 to 40, compiled classes from 655 to 603, and registered admin hooks from 169 to 61.
- **On the `Allowed memory size exhausted` fatals:** on that same install the plugin accounts for 6.6% of a 128 MB limit with the module on, and 3.5% with it off. WordPress core is ~30% before any plugin loads, and a production site adds a theme and other plugins on top. So this removes a real, measurable slice of the pressure, but it is **risk reduction, not a guaranteed fix** — if fatals persist, the remaining levers are raising the PHP memory limit or reducing other plugin load. The settings page now shows your actual peak so you can judge this on your own host.
...
- With the module off, nine shortcodes are no longer registered and render as nothing: `[tournament_clock]`, `[tdwp_tournament_clock]`, `[tdwp_live_clock]`, `[tdwp_leaderboard]`, `[tdwp_tournament_display]`, `[tdwp_current_blinds]`, `[tdwp_player_count]`, `[tdwp_prize_pool]` and `[tdwp_screen_preview]`. `[tdwp_player_registration]` stays registered but renders nothing for visitors and a short explanatory note for administrators. All 39 statistics shortcodes keep working, verified on a real install.
