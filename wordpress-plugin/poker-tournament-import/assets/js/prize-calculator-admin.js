...
(function ($) {
  ...
  var TDWPPrizeCalc = {
    ...
    bindEvents: function () {
      ...
      // Fixed-amount / locked changes also update remaining pool (tdwp-cma.24).
      $(document).on('input', '.amount-input', this.updateRemainingPool.bind(this));
      $(document).on('change', 'input[name$="[locked]"]', this.updateRemainingPool.bind(this));
    ...
    /**
     * Update the "remaining pool" indicator (tdwp-cma.24).
     *
     * Shows how much of the example pool is still available for unlocked
     * percentage-based places after deducting locked/fixed amounts.
     *
     * @since 3.7.1
     */
    updateRemainingPool: function () {
      var $amount = $('#remaining-pool-amount');
      var $status = $('#remaining-pool-status');

      if (!$amount.length) {
        return; // indicator not present on this page (e.g. calculator tab).
      }

      var examplePool =
        typeof tdwpPrizeCalc !== 'undefined' && tdwpPrizeCalc.i18n.examplePool
          ? parseFloat(tdwpPrizeCalc.i18n.examplePool)
          : 10000;
      ...
      $('.place-row').each(function () {
        var $row = $(this);
        var fixedAmount = parseFloat($row.find('.amount-input').val()) || 0;
        var isLocked = $row.find('input[name$="[locked]"]').is(':checked');
        var percentage = parseFloat($row.find('.percentage-input').val()) || 0;

        if (fixedAmount > 0) {
          // Fixed dollar amount — deduct directly.
          lockedTotal += fixedAmount;
        } else if (isLocked) {
          // Locked percentage — deduct that share of the example pool.
          lockedTotal += examplePool * (percentage / 100);
        }
      });

      var remaining = examplePool - lockedTotal;
      var currency = typeof tdwpPrizeCalc !== 'undefined' ? tdwpPrizeCalc.currency : '$';

      $amount.text(currency + remaining.toFixed(2));

      if (remaining < -0.005) {
        // Over-allocated.
        $amount.addClass('over-allocated').removeClass('within-pool');
        $status
          .text(
            typeof tdwpPrizeCalc !== 'undefined'
              ? tdwpPrizeCalc.i18n.overAllocated
              : 'Over-allocated'
          )
          .addClass('over-allocated')
          .removeClass('within-pool');
      } else {
        $amount.removeClass('over-allocated').addClass('within-pool');
        $status.text('').removeClass('over-allocated').addClass('within-pool');
      }
    },
    ...
    loadPreset: function (e) {
      ...
      preset.places.forEach(function (placeData) {
        ...
        var $row = $(
          template
            .replace(/__PLACE__/g, placeData.place)
            .replace(/__PERCENTAGE__/g, placeData.percentage)
        );
        $('#places-container').append($row);
        self.placeCounter++;
      });
    ...
    addChopPlayer: function () {
      ...
      html +=
        '<input type="text" class="player-name" placeholder="Player Name" value="Player ' +
        playerNum +
        '">';
      html +=
        '<input type="number" class="player-chips" placeholder="Chips" min="0" value="10000">';
