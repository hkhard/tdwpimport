...
jQuery(document).ready(function ($) {
  ...
  /**
   * Test Formula button handler — validates the expression and runs a
   * sample calculation via the tdwp_validate_formula AJAX handler.
   */
  $('#test-formula-btn').click(function (e) {
    e.preventDefault();

    var expression = $('#formula-expression').val().trim();
    var $result = $('#formula-test-result');
    ...
    // Fold any dependency lines into the formula so dependent variables
    // (e.g. avgBC) resolve, mirroring how the importer assembles formulas.
    var deps = $('#formula-dependencies').val().trim();
    var fullFormula = deps
      ? deps
          .split('\n')
          .map(function (s) {
            return s.trim();
    ...
    $.ajax({
      ...
      type: 'POST',
      data: {
        action: 'tdwp_validate_formula',
        nonce: pokerFormulaManager.validateNonce,
        formula: fullFormula,
        test_data: { n: 20, r: 3, hits: 5, total_money: 2000, total_buyins: 20 },
      },
      success: function (response) {
        // The handler echoes an HTML fragment, not JSON.
        $result.html(response);
      },
      error: function (xhr, status, error) {
        $result.html('<div class="error"><p>Validation request failed: ' + error + '</p></div>');
      },
    });
  });
