...
(function ($) {
  ...
  var TDWPPlayerMgmt = {
    ...
    bindEvents: function () {
      ...
      // Search players
      $('.tdwp-list-search input[type="text"]').on(
        'keyup',
        this.debounce(this.searchPlayers.bind(this), 300)
      );

      // Promote a waitlisted player to confirmed (tdwp-cma.27).
      $(document).on('click', '.tdwp-promote-waitlisted', this.promoteWaitlisted.bind(this));

      // Copy roster from a previous tournament (tdwp-cma.8).
      $(document).on('click', '.tdwp-copy-roster-button', this.copyRoster.bind(this));
    },
    ...
    promoteWaitlisted: function (e) {
      ...
      $.ajax({
        ...
        type: 'POST',
        data: {
          action: 'tdwp_promote_waitlisted_player',
        ...
        success: function (response) {
          if (response.success) {
            ...
            alert(response.data.message || tdwpPlayerMgmt.i18n.errorSaving);
            $button.prop('disabled', false).removeClass('is-busy');
          }
        },
        error: function () {
          alert(tdwpPlayerMgmt.i18n.errorSaving);
          $button.prop('disabled', false).removeClass('is-busy');
        },
      });
    },
    ...
    /**
     * Copy a roster from a previous tournament into the current one.
     *
     * @since 3.7.0
     * @param {Event} e Click event
     */
    copyRoster: function (e) {
      e.preventDefault();

      var $button = $(e.currentTarget);
      var targetId = $button.data('target-id');
      var sourceId = $('.tdwp-copy-roster-source').val();
      var $result = $('.tdwp-copy-roster-result');

      if (!sourceId || sourceId === '0') {
        $result.text(tdwpPlayerMgmt.i18n.selectSource);
        return;
      }
      ...
      $.ajax({
        ...
        type: 'POST',
        data: {
          action: 'tdwp_copy_player_roster',
          nonce: tdwpPlayerMgmt.nonce,
          source_id: sourceId,
          target_id: targetId,
        },
        success: function (response) {
          if (response.success) {
            ...
            $result.text(response.data.message || tdwpPlayerMgmt.i18n.errorSaving);
          }
          $button.prop('disabled', false).removeClass('is-busy');
        },
        error: function () {
          $result.text(tdwpPlayerMgmt.i18n.errorSaving);
          $button.prop('disabled', false).removeClass('is-busy');
        },
      });
    },
    ...
    displayImportPreview: function (data) {
      ...
      if (duplicates.length > 0) {
        ...
        html +=
          '<p>Based on your duplicate handling setting, these will be skipped or updated.</p>';
        html += '</div>';
      }
    ...
    displayImportResults: function (results) {
      ...
      html +=
        ...
        '?post_type=tournament&page=tdwp-player-management&tab=list" class="button button-primary">View Players List</a></p>';
