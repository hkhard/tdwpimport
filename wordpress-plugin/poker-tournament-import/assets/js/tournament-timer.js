...
(function ($) {
  ...
  const TournamentTimer = {
    ...
    skipLevel: function () {
      const target = parseInt(prompt('Skip to which level?'), 10);
      ...
      this.ajaxCall(
        ...
        (response) => {
          ...
          if (response && response.state && response.state.time_remaining !== undefined) {
            this.timeRemaining = parseInt(response.state.time_remaining, 10);
          }

          this.updateLevelDisplay();

          if (window.TournamentControl) {
            window.TournamentControl.showNotice('Skipped to level ' + this.currentLevel);
          }
        }
      );
    },
    ...
    updateControls: function () {
      ...
      $('#btn-finish')
        .toggle(this.status !== 'finished')
        .prop('disabled', this.status === 'finished');
