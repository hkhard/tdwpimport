...
* New setting, Poker Import → Settings → Feature Modules, to turn the Tournament Manager on or off. It is OFF by default. With it off the plugin skips loading the entire live-play and display-screen subsystem: measured against the previous release, 4 MB less peak memory and 108 fewer registered hooks per admin page load (2 MB less on the front end). Importing .tdt files, the dashboard, leaderboards, season and series standings, and player profiles are unaffected. No data is deleted: your live tournament tables are kept, so you can switch the module back on at any time.
* Note on "Allowed memory size exhausted" errors: this cuts the plugin's own share of a 128 MB PHP limit from roughly 7.8% to 4.7%. That is genuine headroom, but the plugin was never the bulk of the memory in use, so this reduces the risk rather than guaranteeing those errors are gone. If they continue, the next steps are raising the PHP memory limit or reducing load from other plugins.
* While the module is off, ten live-tournament shortcodes stop rendering ([tournament_clock], [tdwp_tournament_clock], [tdwp_live_clock], [tdwp_leaderboard], [tdwp_tournament_display], [tdwp_current_blinds], [tdwp_player_count], [tdwp_prize_pool], [tdwp_screen_preview], [tdwp_player_registration]) and the Live Tournaments post type is hidden. All 39 statistics shortcodes keep working.
...
* The settings page now shows how much memory the current request used against your PHP limit, so you can see the real headroom on your host.
...
* Security: Audited all 11 unauthenticated AJAX handlers (nonce/capability/sanitization/escaping/prepare) - no CRITICAL/HIGH gaps; one LOW (player-registration rate-limiting) tracked as follow-up (tdwp-vyz)
